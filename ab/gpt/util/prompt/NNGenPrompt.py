    ...
    def get_raw_dataset(self, only_best_accuracy, n_training_prompts=None) -> DataFrame:
        ...
        for key in prompt_dict.keys():
            ...
            if is_pruning:
                ...
                if use_join:
                    # TODO: Generalize for all scenarios - SQL query implementation in the NN Dataset project
                    patch_join_nn_query()
                data = lemur.data(
                ...
                if use_join and key_dict.get('output_type') == 'classification':
                    # TODO: Generalize for all scenarios based on the formula implementation (see evaluate_delimited_formulas(..))
                    enrich_dataframe(data)
                print(
                    f"[STAT] Fetched {len(data)} records from STAT table for key: {key}")
            # ==========================================================

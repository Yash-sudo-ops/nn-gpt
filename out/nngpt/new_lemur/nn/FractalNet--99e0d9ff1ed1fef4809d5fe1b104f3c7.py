...
class MultiBranchConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0, element_list=None):
        ...
        self.in_pr = (
            nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=False) if in_channels != out_channels else nn.Identity()
        )
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % (2 ** j) == 0:
                    ...
                    level.append(
                        drop_conv3x3_block(in_ch_ij, out_channels, stride=1, padding=1, bias=False, dropout_prob=dropout_prob)
                    )
                else:
